...
- [角色剪影必须先区分再进视频关键帧](character-silhouette-must-differ-before-video-keyframes.md) — 多角色动漫视频关键帧生成前，必须先检查角色剪影、发型和色块是否足够区分，避免同框角色像复制粘贴。
...
- [可灵视频提示词必须锁定角色动作差异和空间连续性](kling-prompts-need-action-differentiation.md) — 视频分段关键图和可灵提示词必须提前锁定角色动作差异、空间方向、站位顺序和入场时机。
